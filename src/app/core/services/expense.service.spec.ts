...
describe('ExpenseService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({ providers: [
      ...
      { provide: RECURRENCE_EXCEPTION_REPOSITORY, useValue: { getAll: async () => exceptions, put: async (item: RecurrenceException) => { exceptions = exceptions.filter((current) => !(current.seriesType === item.seriesType && current.seriesId === item.seriesId && current.occurrenceDate === item.occurrenceDate)); exceptions.push(item); }, delete: async () => undefined } },
  ...
  it('persiste edições e omissões com ID PocketBase e chave semântica estável', async () => {
    ...
    expect(exceptions[0]).toMatchObject({ seriesType: 'expense', seriesId: created.id, occurrenceDate: '2026-02-28', action: 'override' });
    expect(isPocketBaseId(exceptions[0].id)).toBe(true);
